...
- افزودن کامپوننت به `SharedModule` (در `declarations` و `exports`) برای استفاده مشابه `app-cms-link-to`.
- ثبت `LinkManagementTargetService` به‌عنوان `provider` در `SharedModule` برای رفع خطای DI در ماژول‌هایی مانند `PanelModule` که فقط `SharedModule` را import می‌کنند.

## 2026-03-09 - cms-link-management-to modal wrapper

- ایجاد کامپوننت `CmsLinkManagementToComponent` در `shared` با سلکتور `app-cms-link-management-to` به‌عنوان یک wrapper ساده برای باز کردن `CmsLinkManagementComponent` در یک `MatDialog` (modal).
- ثبت این کامپوننت در `SharedModule` (هم در `declarations` و هم در `exports`) تا مانند سایر shared components در ماژول‌های مختلف قابل استفاده باشد.
- دکمه پیش‌فرض با ظاهر مدرن (Material) و امکان تنظیم متن و آیکن از بیرون (`@Input` ها) پیاده‌سازی شد.

## 2026-03-09 - cms-link-management direct options & upload

- افزودن ورودی‌های مستقیم به `CmsLinkManagementComponent`:
  - `optionFileId` برای مقداردهی مستقیم `uploadFileGUID` (linkFileId).
  - `optionUrl` برای ساخت لینک کوتاه از یک URL از پیش تعیین‌شده.
  - `optionMessage` برای ساخت لینک کوتاه از متن پیام آماده.
  - `optionKey` برای دریافت لینک بر اساس کلید آماده.
- اگر هر یک از گزینه‌های بالا مقداردهی شود، فقط تب مرتبط فعال و تب‌های دیگر مخفی می‌شوند (برای تجربه متمرکز).
- افزودن امکان **آپلود فایل** داخل تب «ارسال فایل» با استفاده از `app-cms-file-uploader` (مشترک با ماژول application) و ست خودکار `uploadFileGUID` بعد از موفقیت آپلود.
- تنظیم `CmsLinkManagementToComponent` برای پاس‌دادن این گزینه‌ها از طریق `MatDialog` به `CmsLinkManagementComponent` (چهار ورودی: `optionFileId`, `optionUrl`, `optionMessage`, `optionKey`).

## 2026-03-09 - sass import deprecation

- در `cms-link-management.component.scss` استفاده از `@import` که در Sass جدید deprecate شده بود، به `@use "../cms-link-to/cms-link-to.component.scss" as *;` تغییر کرد تا هشدار `Deprecation [plugin angular-sass]` در بیلد از بین برود.

## 2026-03-09 - shared captcha component

- ایجاد کامپوننت مشترک `CmsCaptchaComponent` در `shared` با سلکتور `app-cms-captcha` که شامل نمایش تصویر کپچا و ورودی عددی ۵ رقمی (`ng-otp-input`) است.
- ورودی‌ها:
  - `imageSrc` برای آدرس تصویر کپچا.
  - `config` برای تنظیمات `ng-otp-input` (طول، فقط عدد، استایل و ...).
- خروجی‌ها:
  - `refresh` برای درخواست رفرش تصویر کپچا (کلیک روی تصویر).
  - `codeChange` برای ارسال مقدار کد وارد شده به والد.
- استفاده از این کامپوننت در `CmsLinkManagementComponent` در تمام تب‌ها (ارسال فایل، دریافت لینک، ارسال لینک، ارسال پیام) برای یکپارچه‌سازی UX کپچا و آماده‌سازی جهت استفاده مجدد در سایر بخش‌ها (مثل لاگین و تأیید موبایل).
...
### 2026-03-09 (LinkManagement TargetBillboardLog mobile list align)

- Change: Updated `link-management/target-billboard-log/list/list.mobile.component.html` mobile card layout to show the same key fields as desktop list (id, ipAddress, clickPrice, viewPrice, linkManagementBillboardId, linkManagementTargetId, createdDate, deviceId, browserName) using the standard `app-cms-html-list-mobile` pattern and `fieldsInfo` titles.
- Reason: Align mobile list parameters and UX with desktop list and News mobile standard.

### 2026-03-23 10:02:36 (Shared Professional File Manager)

- Added `app-cms-file-manager` in `src/app/shared/cms-file-manager` as a professional Windows-style wrapper over `cms-file-manager`.
- Implemented advanced UX capabilities: grid/list mode, multi-select, selected-item action bar, copy-link, download-selected, and keyboard shortcuts (`Ctrl+R`, `Ctrl+L`, `Esc`).
- Integrated with existing NTK file-manager API flow (`TreeModel` / current file/category endpoints) without breaking current module usage.
- Updated `src/app/shared/shared.module.ts` to declare and export `CmsFileManagerShellComponent`.
- Added multilingual UI keys for new file-manager shell actions in all i18n files (`fa`, `en`, `ar`, `de`, `es`, `fr`, `ja`, `tr`, `zh`).

### 2026-03-23 10:02:36 (Shared File Manager - Rebuilt from scratch)

- Reworked `app-cms-file-manager` to **remove dependency** on `cms-file-manager` and implement a fully custom module/component behavior.
- Implemented direct API architecture using `FileCategoryService` and `FileContentService` for tree navigation, folder/file listing, create, rename, delete, and refresh.
- Integrated upload flow with shared uploader (`app-cms-file-uploader`) and file registration in current folder.
- Added independent explorer UI: left tree panel, breadcrumbs, search, list/grid modes, multi-select, per-item actions, and bulk actions.
- Extended i18n labels for the new from-scratch UI actions.

### 2026-03-23 10:02:36 (Shared File Manager - Professional completion)

- Completed advanced explorer behavior: back/forward history with keyboard support (`Alt+Left`, `Alt+Right`).
- Added professional sorting controls (field selector + asc/desc toggle) on folder/file listing.
- Added desktop-style context menu on right click with open/download/rename/copy/delete actions.
- Implemented drag-and-drop move between folders for files and folders via API edit operations.
- Expanded i18n labels for new context/sorting actions and validated all locale JSON files.

### 2026-03-27 17:23:22 (News Add file manager replacement)

- Replaced `cms-file-manager` usage with `app-cms-file-manager` in `news/content/add/add.component.html` for:
  - `linkMainImageId`
  - `linkFilePodcastId`
  - `linkFileMovieId`
- Updated `news/content/add/add.component.ts` to remove `ntk-cms-filemanager` imports and accept new file manager selection model.
- Lint check: no errors in changed files.

### 2026-03-27 17:23:22 (Shared File Manager popup mode)

- Added Popup/Modal capability to `app-cms-file-manager` using the same UX pattern as shared “to” components (openForm two-way binding + overlay).
- New inputs: `isPopup`, `openFilemanagerButtonView`, `openFilemanagerButtonLabelKey`, plus close behavior on backdrop/Esc.
- Added i18n keys for popup open/close labels and validated templates with lint checks.
