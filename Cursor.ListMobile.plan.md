...
## Part 34: بهبود UX لیست موبایل لینک تارگت (2026-03-09)

- بازنویسی `link-management/target/list/list.mobile.component.html` بر اساس الگوی استاندارد `app-cms-html-list-mobile` و هم‌خوان با `link-management/billboard`.
- اصلاح شرط‌ها از `tableRowSelected?.id?.length > 0` به `tableRowSelected?.id > 0` برای جلوگیری از خطای منطقی روی شناسه عددی.
- حذف `type="button"` از تمام تگ‌های `<a>` در این فایل مطابق استاندارد Part 33.
- غنی‌سازی نمای موبایل با نمایش تصویر بندانگشتی، سایت مقصد، تعداد نمایش/کلیک، تاریخ ایجاد و بخش expanded شامل:
  - کپی سریع شناسه
  - نمایش لینک اصلی (`toWebAddress`) با دکمه کپی و لینک قابل کلیک
  - نمایش لینک کوتاه (`carryToWebAddress`) با دکمه کپی و لینک قابل کلیک
  - نمایش فایل‌های پیوست بر اساس `linkFileIds` (نام، حجم، لینک دانلود)

### Result 34: لیست موبایل تارگت به‌روز شد – 2026-03-09

- UX موبایل برای لیست تارگت با ظاهر کارت‌محور مدرن و سازگار با سایر لیست‌های موبایل لینک‌منیجمنت یکسان شد.
- تمام شرط‌های نمایش اکشن‌ها و نوار اکشن‌رو بر اساس `id > 0` اصلاح شد و مشکل عدم نمایش اکشن روی ردیف انتخابی برطرف شد.
- جزئیات مهم (لینک اصلی، لینک کوتاه، فایل‌های پیوست) در حالت expanded به‌صورت فشرده و مناسب موبایل در دسترس کاربر قرار گرفت.
- لاینتر روی فایل‌های `list.mobile.component.html` و `list.mobile.component.ts` برای این مسیر اجرا شد و خطایی گزارش نشد.
