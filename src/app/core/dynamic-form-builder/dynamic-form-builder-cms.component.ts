...
export class DynamicFormBuilderCmsComponent implements OnInit, AfterViewInit {
  ...
  @Input() set jsonValue(model: string) {
    if (model && model.length > 0) {
      ...
      this.theKeys = Object.getOwnPropertyNames(
        this.privateJsonValue,
      ).toString();
  ...
  ngAfterViewInit(): void {
    this.unsubcribe = this.formGroup.valueChanges
      ...
      .subscribe((update) => {
        ...
        if (
          this.privatePropertiesInfos &&
          this.privatePropertiesInfos.length > 0
        ) {
  ...
  actionFormMake() {
    ...
    this.fields.forEach((x) => {
      if (x.type === "checkbox") {
        ...
        x.options.forEach((o) => {
          (this.formGroup.get(x.name) as FormGroup).addControl(
            o.key,
  ...
  actionSetValue(): void {
    if (
      ...
      this.privatePropertiesInfos.forEach((x) => {
        ...
        if (
          this.formValues[x.fieldName] &&
          this.fields.findIndex((y) => y.name === x.fieldName) >= 0
        ) {
