...
export class PublicHelper {
  ...
  editorConfigFull: AngularEditorConfig = {
    ...
    sanitize: false,
    toolbarPosition: "top",
    toolbarHiddenButtons: [["redo"], ["removeFormat"]],
  };
  ...
  GetfileManagerTreeConfig(): TreeModel {
    this.fileManagerTreeConfig.config.baseURL =
      environment.cmsServerConfig.configApiServerPath;
    this.fileManagerTreeConfig.config.baseUploadURL =
      environment.cmsServerConfig.configFileServerPath;
  ...
  fieldInfo(
    ...
    if (
      !dataAccessModel ||
      !dataAccessModel.fieldsInfo ||
      dataAccessModel.fieldsInfo.length === 0
    ) {
      return retOut;
    }
    const field = dataAccessModel.fieldsInfo.find(
  ...
  async getCurrency(): Promise<ErrorExceptionResult<CoreCurrencyModel>> {
    ...
    while (this.getCurrencyActionIndo) {
      ...
      setTimeout(() => {}, 10000);
      i++;
      if (i == 100) this.getCurrencyActionIndo = false;
    }
    const storeSnapshot = this.cmsStoreService.getStateSnapshot();
    if (
      storeSnapshot?.currencyResultStore?.isSuccess &&
      storeSnapshot?.currencyResultStore?.listItems?.length > 0
    ) {
    ...
    return await firstValueFrom(
      this.coreCurrencyService.ServiceGetAll(null),
    ).then((response) => {
      this.getCurrencyActionIndo = false;
      this.cmsStoreService.setState({
        type: SET_Core_Currency,
        payload: response,
      });
  ...
  async getEnumRecordStatus(): Promise<ErrorExceptionResult<InfoEnumModel>> {
    ...
    while (this.getEnumRecordStatusActionIndo) {
      ...
      setTimeout(() => {}, 100000);
      i++;
      if (i == 100) this.getEnumRecordStatusActionIndo = false;
    }
    const storeSnapshot = this.cmsStoreService.getStateSnapshot();
    ...
    return await firstValueFrom(
      this.coreEnumService.ServiceRecordStatusEnum(1000000),
    )
      .then((response) => {
  ...
  checkModule(moduleName: string): boolean {
    if (
      environment.cmsServerConfig.modules &&
      environment.cmsServerConfig.modules.length > 0 &&
      environment.cmsServerConfig.modules.findIndex(
        (x) => x.toLowerCase() == moduleName.toLowerCase(),
      ) < 0
    )
      return false;
    return true;
  ...
  TableDisplayedColumns(
    cloumnDesktopSource: string[],
    cloumnMobileDispalySource: string[],
    cloumnAdminAccessDispaly: string[],
    token: TokenInfoModelV3,
  ): string[] {
    ...
    var cloumn: string[] = [];
    if (
      window.innerWidth < environment.cmsViewConfig.mobileWindowInnerWidth &&
      cloumnMobileDispalySource &&
      cloumnMobileDispalySource.length > 0
    ) {
    ...
    if (
      token.access &&
      (token.access.userAccessAdminAllowToAllData ||
        token.access.userAccessAdminAllowToProfessionalData)
    ) {
      var i = 0;
  ...
  InfoPerviousRowInList<TKey>(
    list: BaseEntity<TKey>[],
    item: BaseEntity<TKey>,
  ): BaseEntity<TKey> {
  ...
  setComponentLocalStorage(
    name: string,
    model: ComponentLocalStorageModel,
  ): void {
  ...
  getComponentLocalStorage(name: string): ComponentLocalStorageModel {
    ...
    if (lStor) retOut = JSON.parse(lStor);
    if (!retOut) retOut = new ComponentLocalStorageModel();
    return retOut;
  }
  setComponentLocalStorageMap(name: string, key: string, value: any): void {
    var lStor = this.getComponentLocalStorage(name);
    if (!lStor.componentValues) lStor.componentValues = new Map();
  ...
  getComponentLocalStorageMap(name: string, key: string): string {
    ...
    if (lStor) model = JSON.parse(lStor);
    if (model && model.componentValues) return model.componentValues[key];
    return "";
  }
}
