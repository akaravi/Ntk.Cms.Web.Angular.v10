...
export class ProcessService {
  constructor(public cmsStoreService: CmsStoreService) {
    ...
    this.processSubject.subscribe(() => {
      try {
        if (this.cdr) Promise.resolve().then(() => this.cdr.detectChanges());
      } catch (error) {
  ...
  */
  public processStart(
    key: string,
    title: string = "",
    infoAreaId: string = "global",
  ): void {
    let model = new ProcessInfoModel();
    if (!title || title.length === 0) title = "loading data";
    model.isComplate = false;
    model.title = title;
    model.infoAreaId = infoAreaId;
    if (environment.ProgressConsoleLog)
      console.log(
        "#### processStart #### " + key + " " + JSON.stringify(model),
      );
  ...
  public processStop(key: string, isSuccess = true): void {
    ...
    for (const [key, value] of this.process.infoAll) {
      ...
      if (
        value &&
        value.isComplate === false &&
        value.infoAreaId === model.infoAreaId
      ) {
