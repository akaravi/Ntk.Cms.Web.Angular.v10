...
export class CmsSignalrService {
  ...
  public async initiateSignalrConnection(): Promise<void> {
    ...
    //var messageHeaders: signalR.MessageHeaders = { "deviceToken": deviceToken };
    try {
      ...
      await this.hubConnection
        .start()
        .then(() => {
          if (environment.consoleLog) console.log("signalR Connection started");
          console.log(
        ...
        .catch((err) =>
          console.log("Error while signalR starting connection: " + err),
        );
    } catch (error) {
  ...
  public addListenerMessage = (xFunc: any) => {
    if (this.hubConnection)
      this.hubConnection.on(
        "ActionSendMessageToClient",
        (notification: CmsNotificationModel) => {
          if (notification.title?.length > 0) {
            notification.title =
              notification.title + " " + new Date().toLocaleTimeString();
  ...
  public addListenerActionLogout = () => {
    if (this.hubConnection)
      this.hubConnection.on(
        "ActionLogout",
        (model: ErrorExceptionResultBase) => {
