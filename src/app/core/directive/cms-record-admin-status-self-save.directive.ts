 ...
 */
@Directive({
  selector: "[cmsRecordAdminStatusSelfSave]",
  standalone: false,
})
export class CmsRecordAdminStatusSelfSaveDirective {
  @Input("row")
  row: any;
  ...
  constructor(
    ...
    this.DataEnumRecordAdminStatus();
  }
  async DataEnumRecordAdminStatus(): Promise<void> {
    const dataModelResult = await this.publicHelper.getEnumRecordAdminStatus();
