...
export class OperatorsService {
  ...
  getOperators(rule: QueryRule, fieldMap: QueryBuilderFieldMap): string[] {
    if (!this.getAvailableTypes().includes(rule.type)) {
      console.error(
        "Type of rule is not valid. Please use one of the following types: " +
          this.getAvailableTypes().join(", "),
      );
    }
    ...
    // allow nullable
    if (
      fieldMap[rule.field].settings &&
      fieldMap[rule.field].settings.nullableOperators
    ) {
    ...
    if (
      fieldMap[rule.field].settings &&
      fieldMap[rule.field].settings.emptyOperators
    ) {
