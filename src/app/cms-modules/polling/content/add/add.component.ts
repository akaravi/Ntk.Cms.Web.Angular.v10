...
import {
    ...
    FilterDataModel,
    FilterModel, PollingCategoryModel,
    PollingContentModel,
...
export class PollingContentAddComponent
  extends AddBaseComponent<PollingContentService, PollingContentModel, number>
  implements OnInit
{
  requestCategoryId = 0;
  ...
  dataModelResult: ErrorExceptionResult<PollingContentModel> =
    new ErrorExceptionResult<PollingContentModel>();
  ...
  dataOptionModelResult: ErrorExceptionResult<PollingOptionModel> =
    new ErrorExceptionResult<PollingOptionModel>();
  optionActionTitle = "";
  ...
  DataAddContent(): void {
    ...
    this.pollingContentService.ServiceAdd(this.dataModel).subscribe({
      next: (ret) => {
        this.publicHelper.processService.processStop(pName);
        this.dataModelResult = ret;
        if (ret.isSuccess) {
  ...
  onActionOptionAddToList(): void {
    ...
    if (
      this.optionDataModel.find((x) => x.option === this.optionSelected.option)
    ) {
