...
export class EstateAccountExpertSelectorComponent implements OnInit {
  ...
  @Input() set optionSelectForce(
    x: string | number | EstateAccountExpertModel,
  ) {
  ...
  ): Promise<EstateAccountExpertModel[]> {
  ...
  push(
    newvalue: EstateAccountExpertModel,
  ): Observable<EstateAccountExpertModel[]> {
  ...
  onActionSelectForce(id: string | number | EstateAccountExpertModel): void {
    if (typeof id === "number" && id > 0) {
      ...
      if (
        ...
        this.dataModelResult.listItems.find((x) => x.linkCmsUserId === id)
      ) {
        const item = this.dataModelResult.listItems.find(
          (x) => x.linkCmsUserId === id,
        );
