...
export class EstatePropertyHistoryWidgetComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.translate.get("ROUTE.ESTATE.HISTORY").subscribe((str: string) => {
  ...
  onActionStatist(): void {
    ...
    this.widgetInfoModel.setItem(
      new WidgetContentInfoModel("InChecking", 0, 0, ""),
    );
    this.widgetInfoModel.setItem(
      new WidgetContentInfoModel("Available", 1, 0, ""),
    );
    this.widgetInfoModel.setItem(new WidgetContentInfoModel("All", 2, 0, ""));
    this.widgetInfoModel.setItem(
      new WidgetContentInfoModel("Planned", 3, 0, ""),
    );
    this.widgetInfoModel.setItem(
      new WidgetContentInfoModel("PlannedToDay", 4, 0, ""),
    );
    ...
    const hours = 48;
    filterStatist4.onDateTimeFrom = new Date(
      new Date().setHours(0, 0, 0, 0) - hours * 60 * 60 * 1000,
    );
    filterStatist4.onDateTimeTo = new Date(
        ...
        if (ret.isSuccess) {
          if (ret.totalRowCount > 0) {
            ...
            this.cmsToastrService.typeWarningMessage(
              "تعداد " +
                ret.totalRowCount +
                " عدد فعالیت برای امروز شما می باشد",
              " فعالیت برنامه ریزی شده برای شما",
            );
            setTimeout(
  ...
  confirmDialogPlanned(count: number) {
    ...
    var message = "";
    this.translate
      .get(["MESSAGE.Please_Confirm", "MESSAGE.Do_you_want_to_View"])
      .subscribe((str: string) => {
        title = str["MESSAGE.Please_Confirm"];
        message =
          str["MESSAGE.Do_you_want_to_View"] +
          "?" +
          "<br> ( " +
          "تعداد " +
          count +
