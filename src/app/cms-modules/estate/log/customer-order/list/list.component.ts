...
export class EstateCustomerOrderListComponent
  ...
  implements OnInit, OnDestroy
{
  ...
  DataGetAll(): void {
    ...
    if (this.requestLinkPropertyId && this.requestLinkPropertyId.length > 0) {
      ...
      this.requestLinkPropertyIdHaveHistory &&
      this.requestLinkPropertyIdHaveHistory.length > 0
    ) {
  ...
  onActionButtonHistoryRow(
    mode: EstateCustomerOrderModel = this.tableRowSelected,
    event?: MouseEvent,
  ): void {
  ...
  onActionButtonStatist(view = !this.optionsStatist.data.show): void {
    ...
    forkJoin([s0, s1, s2, s3, s4]).subscribe((results) => {
      ...
      if (ret.isSuccess) {
        this.translate
          .get("MESSAGE.customer_order_list")
          .subscribe((str: string) => {
  ...
  onActionButtonLinkTo(
    ...
    this.translate
      .get("MESSAGE.get_customer_information")
      .subscribe((str: string) => {
        this.publicHelper.processService.processStart(
          pName,
          str,
