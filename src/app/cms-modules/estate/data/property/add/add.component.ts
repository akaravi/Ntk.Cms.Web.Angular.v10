...
export class EstatePropertyAddComponent
  ...
  implements OnInit, OnDestroy
{
  ...
  dataModelEstatePropertyTypeResult: ErrorExceptionResult<EstatePropertyTypeModel> =
    new ErrorExceptionResult<EstatePropertyTypeModel>();
  ...
  getEstatePropertyType(): void {
    ...
    this.translate
      .get("TITLE.Get_Estate_property_Type")
      .subscribe((str: string) => {
        this.publicHelper.processService.processStart(
          pName,
          str,
  ...
  getEstatePropertyTypeLanduse(): void {
    const pName = this.constructor.name + "getEstatePropertyType";
    this.translate
      .get("TITLE.Get_Estate_user_Type")
      .subscribe((str: string) => {
        this.publicHelper.processService.processStart(
          pName,
          str,
  ...
  DataAdd(): void {
    ...
    this.translate
      .get("TITLE.Property_registration")
      .subscribe((str: string) => {
        this.publicHelper.processService.processStart(
          pName,
          str,
  ...
  onActionSelectorSelectUsage(
    ...
    if (
      this.dataModelEstatePropertyTypeResult.isSuccess &&
      this.dataModelEstatePropertyTypeLanduseResult.isSuccess
    ) {
      ...
      this.dataModelEstatePropertyTypeResult.listItems.forEach((element) => {
        if (element.linkPropertyTypeUsageId === model.id) {
          this.dataModelEstatePropertyTypeLanduseResult.listItems.forEach(
            (elementLanduser) => {
  ...
  onStepClick(event: StepperSelectionEvent, stepper: MatStepper): void {
    ...
    if (
      (!this.dataModel.contracts || this.dataModel.contracts.length == 0) &&
      event.previouslySelectedStep.state == "contract" &&
      event.previouslySelectedIndex < event.selectedIndex
    ) {
      this.translate
        .get("TITLE.Select_the_transaction_type")
        .subscribe((str: string) => {
          this.cmsToastrService.typeErrorFormInvalid(str);
        });
