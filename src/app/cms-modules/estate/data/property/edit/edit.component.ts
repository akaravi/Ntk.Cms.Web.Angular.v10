...
export class EstatePropertyEditComponent
  extends EditBaseComponent<EstatePropertyService, EstatePropertyModel, string>
  implements OnInit, OnDestroy
{
  ...
  @ViewChild(EstateAccountAgencyListComponent)
  estateAccountAgencyListComponent: EstateAccountAgencyListComponent;
  @ViewChild(EstateAccountExpertListComponent)
  estateAccountExpertListComponent: EstateAccountExpertListComponent;
  @ViewChild(EstateCustomerOrderListComponent)
  estateCustomerOrderListComponent: EstateCustomerOrderListComponent;
  @ViewChild(EstateCustomerOrderListComponent)
  estateCustomerOrderHaveHistoryListComponent: EstateCustomerOrderListComponent;
  @ViewChild(EstatePropertyHistoryListComponent)
  estatePropertyHistoryListComponent: EstatePropertyHistoryListComponent;
  ...
  dataModelEstateContractTypeResult: ErrorExceptionResult<EstateContractTypeModel> =
  ...
  getEstateContractType(): void {
    ...
    this.translate
      .get("TITLE.Get_Estate_Contract_Type")
      ...
      next: (ret) => {
        ...
        if (ret.isSuccess) {
          if (!this.dataModel.caseCode || this.dataModel.caseCode.length <= 0)
            this.dataModel.caseCode = this.publicHelper.StringRandomGenerator(
              5,
              true,
            );

          this.optionTabledataSource.data = this.dataModel.contracts;
          this.DataGetPropertyDetailGroup(
          ...
          if (
            this.dataModel.linkExtraImageIdsSrc &&
            this.dataModel.linkExtraImageIdsSrc.length > 0
          ) {
            this.dataModel.linkExtraImageIds
              .split(",")
              .forEach((element, index) => {
                let link = "";
                if (
                  this.dataModel.linkExtraImageIdsSrc.length >=
                  this.dataModel.linkExtraImageIdsSrc.length
                ) {
  ...
  DataGetPropertyDetailGroup(id: string): void {
    ...
    this.translate
      .get("MESSAGE.Get_detailed_information")
      .subscribe((str: string) => {
        this.publicHelper.processService.processStart(
          pName,
          str,
  ...
  DataEditContent(forcePopupMessageAction = false): void {
    ...
    this.translate
      .get("MESSAGE.registration_chaneges_in_property_information")
      .subscribe((str: string) => {
        this.publicHelper.processService.processStart(
          pName,
          str,
  ...
  onActionSelectorContractType(model: EstateContractTypeModel | null): void {
    ...
    this.contractDataModel.linkEstateContractTypeId =
      this.contractTypeSelected.id;
  }
  ...
  onActionOptionAddToList(viewAlert: boolean = true): void {
    if (
      ...
      this.contractTypeSelected.id.length === 0
    ) {
      this.translate
        .get("MESSAGE.Type_of_property_transaction_is_not_known")
        .subscribe((str: string) => {
    ...
    if (this.contractTypeSelected.hasSalePrice) {
      if (
        this.contractDataModel.salePrice &&
        this.contractDataModel.salePrice > 0
      )
        accepted = true;
      if (
        this.contractTypeSelected.salePriceAllowAgreement &&
        this.contractDataModel.salePriceByAgreement
      )
        accepted = true;

      if (!accepted) {
        this.translate
          .get("MESSAGE.Sales_amount_is_not_entered_correctly")
          .subscribe((str: string) => {
            this.cmsToastrService.typeErrorSelected(str);
          });
        return;
    ...
    if (this.contractTypeSelected.hasRentPrice) {
      if (
        this.contractDataModel.rentPrice &&
        this.contractDataModel.rentPrice > 0
      )
        accepted = true;
      if (
        this.contractTypeSelected.rentPriceAllowAgreement &&
        this.contractDataModel.rentPriceByAgreement
      )
        accepted = true;

      if (!accepted) {
        this.translate
          .get("MESSAGE.Rent_amount_is_not_entered_correctly")
          .subscribe((str: string) => {
            this.cmsToastrService.typeErrorSelected(str);
          });
        return;
    ...
    if (this.contractTypeSelected.hasPeriodPrice) {
      if (
        this.contractDataModel.periodPrice &&
        this.contractDataModel.periodPrice > 0
      )
        accepted = true;
      if (
        this.contractTypeSelected.periodPriceAllowAgreement &&
        this.contractDataModel.periodPriceByAgreement
      )
        accepted = true;

      if (!accepted) {
        this.translate
          .get("MESSAGE.Period_amount_is_not_entered_correctly")
          .subscribe((str: string) => {
            this.cmsToastrService.typeErrorSelected(str);
          });
        return;
    ...
    if (this.contractTypeSelected.hasDepositPrice) {
      if (
        this.contractDataModel.depositPrice &&
        this.contractDataModel.depositPrice > 0
      )
        accepted = true;
      if (
        this.contractTypeSelected.depositPriceAllowAgreement &&
        this.contractDataModel.depositPriceByAgreement
      )
        accepted = true;

      if (!accepted) {
        this.translate
          .get("MESSAGE.Deposit_amount_is_not_entered_correctly")
          .subscribe((str: string) => {
            this.cmsToastrService.typeErrorSelected(str);
          });
        return;
  ...
  onStepClick(event: StepperSelectionEvent, stepper: MatStepper): void {
    if (event.previouslySelectedIndex < event.selectedIndex) {
      if (
        ...
        this.translate
          .get("TITLE.Select_the_Property_Type_Usage")
          .subscribe((str: string) => {
      ...
      if (
        ...
        this.translate
          .get("TITLE.Select_the_Property_Type_Landuse")
          .subscribe((str: string) => {
            this.cmsToastrService.typeErrorFormInvalid(str);
          });
  ...
  onActionSelectCurrency(model: CoreCurrencyModel): void {
    ...
    if (
      this.tokenHelper.isAdminSite &&
      this.contractTypeSelected.allowPriceInquiryCalculate
    ) {
  ...
  onActionButtonQuickListSearchCustomerTel(): void {
    if (
      !this.dataModel ||
      !this.dataModel.aboutCustomerTel ||
      this.dataModel.aboutCustomerTel.length === 0
    ) {
      this.cmsToastrService.typeErrorSelectedRow();
      return;
    }
  ...
  onActionButtonQuickListSearchCustomerMobile(): void {
    if (
      !this.dataModel ||
      !this.dataModel.aboutCustomerMobile ||
      this.dataModel.aboutCustomerMobile.length === 0
    ) {
      this.cmsToastrService.typeErrorSelectedRow();
      return;
    }
  ...
  onActionButtonQuickListSearchCaseCode(): void {
    if (
      !this.dataModel ||
      !this.dataModel.caseCode ||
      this.dataModel.caseCode.length === 0
    ) {
      this.cmsToastrService.typeErrorSelectedRow();
      return;
    }
