...
export class EstatePropertySupplierEditComponent
  ...
  implements OnInit
{
  ...
  similarTabledataSource =
    new MatTableDataSource<EstatePropertySupplierModel>();
  ...
  onActionContentSimilarSelect(
    model: EstatePropertySupplierModel | null,
  ): void {
  ...
  onActionContentSimilarAddToLIst(): void {
    if (
      !this.contentSimilarSelected ||
      this.contentSimilarSelected.id.length <= 0
    ) {
      return;
    }
    if (
