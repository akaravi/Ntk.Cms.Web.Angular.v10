...
import { ActivityStatusEnumEstate, EstatePropertyHistoryFilterModel, EstatePropertyHistoryService, FilterDataModel, FilterDataModelSearchTypesEnum, FilterModel, ManageUserAccessDataTypesEnum, RecordStatusEnum, TokenInfoModelV3 } from 'ntk-cms-api';
...
export class EstatePropertyHistoryWidgetComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.translate.get('ROUTE.ESTATE.HISTORY').subscribe((str: string) => { this.widgetInfoModel.title = str });
  ...
  onActionStatist(): void {
    ...
    filterStatist4.linkResponsibleUserId = this.tokenInfo.access.userId;
    fastfilter = new FilterDataModel();
    fastfilter.propertyName = 'activityStatus';
    fastfilter.value = ActivityStatusEnumEstate.Planned;
    fastfilter.searchType = FilterDataModelSearchTypesEnum.Equal;
    ...
    this.service.ServiceGetCount(filterStatist4).subscribe({
      next: (ret) => {
        if (ret.isSuccess) {
          if (ret.totalRowCount > 0) {
            this.widgetInfoModel.setItem(new WidgetContentInfoModel('PlannedToDay', 5, ret.totalRowCount, '/estate/property-history/InCheckingPlanedToDay/' + this.tokenInfo.access.userId));
  ...
  confirmDialogPlanned(count: number) {
    ...
    this.cmsConfirmationDialogService.confirm(title, message)
      .then((confirmed) => {
        if (confirmed) {
          this.router.navigate(['/estate/property-history/InCheckingPlanedToDay/' + this.tokenInfo.access.userId]);
        }
      }
      )
      .catch(() => {
