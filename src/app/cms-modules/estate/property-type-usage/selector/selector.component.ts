...
export class EstatePropertyTypeUsageSelectorComponent
  implements OnInit, OnDestroy
{
  static nextId = 0;
  id = ++EstatePropertyTypeUsageSelectorComponent.nextId;
  ...
  @Input() set optionSelectForce(x: string | EstatePropertyTypeUsageModel) {
    if (
      ...
      ((typeof x === "string" && x.length > 0) ||
        typeof x === typeof EstatePropertyTypeUsageModel)
    )
  ...
  ): Promise<EstatePropertyTypeUsageModel[]> {
  ...
  push(
    newvalue: EstatePropertyTypeUsageModel,
  ): Observable<EstatePropertyTypeUsageModel[]> {
