...
export class EstatePropertyExpertPriceInquiryListComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.translate
      .get("TITLE.Expert_Price_Inquiry")
    ...
    if (
      this.dataModel.linkLocationId > 0 &&
      this.dataModel.linkCoreCurrencyId > 0 &&
      this.dataModel.createdYaer > 0 &&
      this.dataModel.linkContractTypeId &&
  ...
  DataActionContent(): void {
    ...
    this.estatePropertyExpertPriceService
      .ServicePriceInquiryList(this.dataModel)
      .subscribe({
        next: (ret) => {
          ...
          this.formInfo.submitResultMessageType = FormSubmitedStatusEnum.Success;
            ...
            if (ret.listItems && ret.listItems.length > 0) {
              ...
              this.translate
                .get("MESSAGE.PriceInquiryCalculateNotFind")
                .subscribe((str: string) => {
