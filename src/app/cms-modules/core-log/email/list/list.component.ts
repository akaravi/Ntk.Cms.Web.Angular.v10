...
import {
  CoreEnumService, CoreLogEmailModel, CoreLogEmailService, CoreSiteModel, ErrorExceptionResult, FilterDataModel, FilterModel, InfoEnumModel, RecordStatusEnum, SortTypeEnum
} from 'ntk-cms-api';
...
import { CoreLogEmailEditComponent } from '../edit/edit.component';
...
export class CoreLogEmailListComponent extends ListBaseComponent<CoreLogEmailService, CoreLogEmailModel, string>
  ...
  constructor(
    ...
    super(contentService, new CoreLogEmailModel(), publicHelper, tokenHelper, translate);
  ...
  getEnumSendEmailStatusType(): void {
  ...
  DataGetAll(): void {
    ...
    this.onActionTableRowSelect(new CoreLogEmailModel());
  ...
  onActionButtonViewRow(model: CoreLogEmailModel = this.tableRowSelected): void {
    ...
    const dialogRef = this.dialog.open(CoreLogEmailViewComponent, {
      height: '90%',
  ...
  onActionButtonEditRow(model: CoreLogEmailModel = this.tableRowSelected): void {
    ...
    if (this.publicHelper.isMobile)
      ...
      panelClass = 'dialog-min';
    const dialogRef = this.dialog.open(CoreLogEmailEditComponent, {
      height: '90%',
      panelClass: panelClass,
      enterAnimationDuration: environment.cmsViewConfig.enterAnimationDuration,
      exitAnimationDuration: environment.cmsViewConfig.exitAnimationDuration,
  ...
  onActionButtonDeleteRow(model: CoreLogEmailModel = this.tableRowSelected): void {
    if (!model || !model.id || model.id.length === 0) {
  ...
  onActionButtonViewUserRow(model: CoreLogEmailModel = this.tableRowSelected): void {
  ...
  onActionButtonViewMemberRow(model: CoreLogEmailModel = this.tableRowSelected): void {
    ...
    if (!this.tableRowSelected.linkMemberId || this.tableRowSelected.linkMemberId === '') {
      this.translate.get('MESSAGE.content_does_not_include_member_informations').subscribe((str: string) => {
        this.cmsToastrService.typeErrorSelected(str);
      });
      return;
    }
