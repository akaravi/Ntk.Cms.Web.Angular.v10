...
export class CoreUserClaimContentWidgetStatusComponent
  implements OnInit, OnDestroy
{
  ...
  onActionStatist(): void {
    ...
    this.translate
      .get("TITLE.Verification_of_documents_and_identity")
    ...
    this.service.ServiceClaimCheckCurrent().subscribe({
      next: (ret) => {
        if (ret.isSuccess) {
          ...
          if (
            this.dataModelResult.listItems.find(
              (x) =>
                x.recordStatus !== RecordStatusEnum.Pending && !x.isApproved,
            )
          ) {
          }
        } else {
