...
export class CoreSiteWidgetStatusComponent implements OnInit, OnDestroy {
  ...
  onActionStatist(): void {
    if (
      !this.tokenInfoModel.access.siteId ||
      this.tokenInfoModel.access.siteId <= 0
    ) {
      return;
    }
    ...
    this.widgetInfoModel.setItem(
      new WidgetContentInfoModel("Sub Domain", 3, 0, "", "", "..."),
    );
    this.widgetInfoModel.setItem(
      new WidgetContentInfoModel("Created Date", 4, 0, "", "", "..."),
    );
    this.widgetInfoModel.setItem(
      new WidgetContentInfoModel("Expire Date", 5, 0, "", "", "..."),
    );
    this.service
      .ServiceGetOneById(this.tokenInfoModel.access.siteId)
      .subscribe({
        next: (ret) => {
          if (ret.isSuccess) {
            this.widgetInfoModel.setItem(
            ...
            this.widgetInfoModel.setItem(
              new WidgetContentInfoModel(
                ...
                "",
                "",
                this.persianCalendarService.PersianCalendar(
                  ret.item.createdDate,
                ),
              ),
            );
    ...
    if (model && model.id > 0) {
      ...
      if (model.id !== this.tokenInfoModel.access.siteId) {
        ...
        authModel.userAccessAdminAllowToProfessionalData =
          this.tokenInfoModel.access.userAccessAdminAllowToProfessionalData;
        authModel.userAccessAdminAllowToAllData =
          this.tokenInfoModel.access.userAccessAdminAllowToAllData;
          ...
          next: (ret) => {
            if (ret.isSuccess) {
              if (ret.item.access.siteId === +model.id) {
                ...
                this.translate
                  .get("ERRORMESSAGE.MESSAGE.New_site_acess_denied")
                  .subscribe((str: string) => {
