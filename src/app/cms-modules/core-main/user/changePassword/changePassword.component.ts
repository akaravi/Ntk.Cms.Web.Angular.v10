...
export class CoreUserChangePasswordComponent implements OnInit, OnDestroy {
  ...
  dataModelResult: ErrorExceptionResult<CoreUserModel> =
    new ErrorExceptionResult<CoreUserModel>();
  ...
  onFormSubmit(): void {
    ...
    if (this.tokenInfo.access.userAccessAdminAllowToProfessionalData) {
      if (
        ...
        this.dataModel.oldPassword.length === 0
      ) {
      ...
      if (
        ...
        this.translate
          .get("MESSAGE.Enter_the_previous_password")
          .subscribe((str: string) => {
    ...
    if (
      ...
      this.dataModel.newPassword.length === 0
    ) {
      this.translate
        .get("MESSAGE.Enter_the_new_password")
        .subscribe((str: string) => {
          this.cmsToastrService.typeErrorMessage(str);
        });
    ...
    if (this.dataModel.newPassword !== this.NewPasswordRepeat) {
      this.translate
        .get("MESSAGE.The_new_password_is_equivalent_to_a_duplicate")
        .subscribe((str: string) => {
          this.cmsToastrService.typeErrorMessage(str);
        });
