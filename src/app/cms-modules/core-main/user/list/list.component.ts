...
export class CoreUserListComponent
  extends ListBaseComponent<CoreUserService, CoreUserModel, number>
  implements OnInit, OnDestroy
{
  ...
  onActionButtonChangePassword(
    ...
    if (
      this.tokenInfo.access.userId != model.id &&
      (this.dataModelResult == null ||
        this.dataModelResult.access == null ||
        !this.dataModelResult.access.accessEditRow)
    ) {
    ...
    this.translate
      ...
      .subscribe((str: string) => {
        ...
        message =
          ...
          "-" +
          this.tableRowSelected.lastName +
          " ) ";
      });
  ...
  onActionButtonStatist(view = !this.optionsStatist.data.show): void {
    ...
    const filterModel = this.filterModelCompiler(this.filteModelContent);
    this.coreUserService.ServiceGetCount(filterModel).subscribe({
      next: (ret) => {
        if (ret.isSuccess) {
          this.translate.get("MESSAGE.All").subscribe((str: string) => {
  ...
  onActionButtonSiteList(
    ...
    if (event?.ctrlKey) {
      ...
      this.router.navigate([
        "/core/site/list/LinkUserId/",
        this.tableRowSelected.id,
      ]);
    }
  }
  onActionButtonSiteByGroupList(
    model: CoreUserModel = this.tableRowSelected,
    event?: MouseEvent,
  ): void {
    if (!model || !model.id || model.id === 0) {
