...
import { NewsCategoryViewComponent } from "../view/view.component";
  ...
  implements OnInit, OnDestroy
{
  ...
  DataGetAll(): void {
    ...
    this.onActionTableRowSelect(new NewsCategoryModel());
    const pName = this.constructor.name + ".ServiceGetAll";
    this.translate
      .get("MESSAGE.get_categories")
  ...
  onActionButtonNewRow(): void {
    ...
    const dialogRef = this.dialog.open(NewsCategoryAddComponent, {
      height: "90%",
  ...
  onActionButtonEditRow(model: NewsCategoryModel = this.tableRowSelected): void {
  ...
  onActionButtonViewRow(model: NewsCategoryModel = this.tableRowSelected): void {
    if (!model || !model.id || model.id === 0) {
      this.cmsToastrService.typeErrorSelectedRow();
      return;
    }
    ...
    const dialogRef = this.dialog.open(NewsCategoryViewComponent, {
      height: "90%",
      panelClass: panelClass,
      enterAnimationDuration: environment.cmsViewConfig.enterAnimationDuration,
      exitAnimationDuration: environment.cmsViewConfig.exitAnimationDuration,
  ...
  onActionButtonDeleteRow(
    model: NewsCategoryModel = this.tableRowSelected,
  ): void {
    if (!model || !model.id || model.id === 0) {
      this.cmsToastrService.typeErrorSelectedRow();
      return;
  ...
  onActionTableRowSelectActionMenu(row: NewsCategoryModel): void {
    this.onActionTableRowSelect(row);
  }
  override onActionTableRowSelect(row: NewsCategoryModel, event: MouseEvent = null): void {
    super.onActionTableRowSelect(row, event);
  }
  onActionTableRowMouseEnter(row: NewsCategoryModel): void {
    if (row) {
      (row as any).expanded = true;
    }
  }
  onActionTableRowMouseLeave(row: NewsCategoryModel): void {
    if (row) {
      (row as any).expanded = false;
    }
  }
  onActionTableRowSelectActionMenuClick(
    row: NewsCategoryModel,
  ): void {
    this.onActionTableRowSelect(row);
  }
  onActionCopied(): void {
