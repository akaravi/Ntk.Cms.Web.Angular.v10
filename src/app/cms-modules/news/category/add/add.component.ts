...
import {CoreEnumService,
  ...
  NewsCategoryService} from "ntk-cms-api";
...
export class NewsCategoryAddComponent
  extends AddBaseComponent<NewsCategoryService, NewsCategoryModel, number>
  implements OnInit
{
  requestParentId = 0;
  ...
  constructor(
    ...
    protected dialogRef: MatDialogRef<NewsCategoryAddComponent>,
    ...
    this.categoryService.ServiceAdd(this.dataModel).subscribe({
      next: (ret) => {
        ...
        if (ret.isSuccess) {
          ...
          this.translate
            .get("TITLE.typeErrorRegistration")
            .subscribe((str: string) => {
              this.formInfo.submitResultMessage = str;
            });
