...
export class NewsCategoryMenuComponent implements OnInit, OnDestroy {
  ...
  DataListSelect() {
    ...
    var findRow = this.dataModelResult.listItems.filter(
      (x) => x.id === this.requestLinkParentId,
    );
    if (
      findRow &&
      findRow.length > 0 &&
  ...
  onActionClickMenu(item: NewsCategoryModel) {
    ...
    if (item.children?.length > 0) {
      this.router.navigate([
        this.routerLinkCategory + "/LinkParentId/",
        item.id,
      ]);
      return;
    }
    this.router.navigate([
      this.routerLinkContect + "/LinkCategoryId/",
      item.id,
    ]);
  }
  scrollToTop() {
