...
import { CrmActivityAddComponent } from "../add/add.component";
import { CrmActivityEditComponent } from "../edit/edit.component";

/**
 * Component لیست Activityهای CRM.
 * این component لیست تمام Activityهای موجود را نمایش می‌دهد و امکان افزودن، ویرایش و حذف را فراهم می‌کند.
 */
@Component({
  selector: "app-crm-activity-list",
  templateUrl: "./list.component.html",
  standalone: false,
})
export class CrmActivityListComponent
  extends ListBaseComponent<CrmActivityService, CrmActivityModel, string>
  implements OnInit, OnDestroy
{
  ...
  constructor(
    ...
    this.requestLinkLeadId =
      this.activatedRoute.snapshot.paramMap.get("LinkLeadId");
    this.requestLinkOpportunityId =
      this.activatedRoute.snapshot.paramMap.get("LinkOpportunityId");
    this.requestLinkDealId =
      this.activatedRoute.snapshot.paramMap.get("LinkDealId");
  ...
  expandedElement: CrmActivityModel | null;
  ...
  DataGetAll(): void {
    ...
    this.onActionTableRowSelect(new CrmActivityModel());
    ...
    if (
      this.requestLinkLeadId &&
      this.requestLinkLeadId.length > 0
    ) {
      const filter = new FilterDataModel();
      filter.propertyName = "linkLeadId";
      filter.value = this.requestLinkLeadId;
      this.filteModelContent.filters.push(filter);
    }
    if (
      this.requestLinkOpportunityId &&
    ...
    if (
      this.requestLinkDealId &&
      this.requestLinkDealId.length > 0
    ) {
      const filter = new FilterDataModel();
      filter.propertyName = "linkDealId";
      filter.value = this.requestLinkDealId;
      this.filteModelContent.filters.push(filter);
    }
  ...
  onActionButtonNewRow(): void {
    ...
    const dialogRef = this.dialog.open(CrmActivityAddComponent, {
      height: "90%",
  ...
  onActionButtonEditRow(
    model: CrmActivityModel = this.tableRowSelected,
  ): void {
    ...
    const dialogRef = this.dialog.open(CrmActivityEditComponent, {
      height: "90%",
      panelClass: panelClass,
      enterAnimationDuration: environment.cmsViewConfig.enterAnimationDuration,
      exitAnimationDuration: environment.cmsViewConfig.exitAnimationDuration,
  ...
  isExpansionDetailRow = (index: number, row: CrmActivityModel) =>
