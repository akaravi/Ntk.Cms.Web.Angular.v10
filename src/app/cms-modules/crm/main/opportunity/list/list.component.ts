...
import { CrmOpportunityAddComponent } from "../add/add.component";
import { CrmOpportunityEditComponent } from "../edit/edit.component";

/**
 * Component لیست Opportunityهای CRM.
 * این component لیست تمام Opportunityهای موجود را نمایش می‌دهد و امکان افزودن، ویرایش و حذف را فراهم می‌کند.
 */
@Component({
  selector: "app-crm-opportunity-list",
  templateUrl: "./list.component.html",
  standalone: false,
})
export class CrmOpportunityListComponent
  extends ListBaseComponent<CrmOpportunityService, CrmOpportunityModel, string>
  implements OnInit, OnDestroy
{
  ...
  constructor(
    public contentService: CrmOpportunityService,
  ...
  @Input() set optionLinkAccountId(id: string) {
    this.requestLinkAccountId = "";
    if (id && id.length > 0) {
  ...
  expandedElement: CrmOpportunityModel | null;
  ...
  onActionButtonNewRow(): void {
    ...
    const dialogRef = this.dialog.open(CrmOpportunityAddComponent, {
      height: "90%",
  ...
  onActionButtonEditRow(
    model: CrmOpportunityModel = this.tableRowSelected,
  ): void {
    ...
    const dialogRef = this.dialog.open(CrmOpportunityEditComponent, {
      height: "90%",
      panelClass: panelClass,
      enterAnimationDuration: environment.cmsViewConfig.enterAnimationDuration,
      exitAnimationDuration: environment.cmsViewConfig.exitAnimationDuration,
  ...
  isExpansionDetailRow = (index: number, row: CrmOpportunityModel) =>
