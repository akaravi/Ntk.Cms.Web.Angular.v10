...
import { CrmContactAddComponent } from "../add/add.component";
import { CrmContactEditComponent } from "../edit/edit.component";

/**
 * Component لیست Contactهای CRM.
 * این component لیست تمام Contactهای موجود را نمایش می‌دهد و امکان افزودن، ویرایش و حذف را فراهم می‌کند.
 */
@Component({
...
export class CrmContactListComponent
  extends ListBaseComponent<CrmContactService, CrmContactModel, string>
  implements OnInit, OnDestroy
{
  ...
  expandedElement: CrmContactModel | null;
  ...
  DataGetAll(): void {
    ...
    this.onActionTableRowSelect(new CrmContactModel());
  ...
  onActionButtonNewRow(): void {
    ...
    const dialogRef = this.dialog.open(CrmContactAddComponent, {
      height: "90%",
  ...
  onActionButtonEditRow(
    model: CrmContactModel = this.tableRowSelected,
  ): void {
    ...
    const dialogRef = this.dialog.open(CrmContactEditComponent, {
      height: "90%",
      panelClass: panelClass,
      enterAnimationDuration: environment.cmsViewConfig.enterAnimationDuration,
      exitAnimationDuration: environment.cmsViewConfig.exitAnimationDuration,
    ...
    this.translate
      ...
      .subscribe((str: string) => {
        ...
        message =
          ...
          "?" +
          "<br> ( " +
          (this.tableRowSelected.fullName || this.tableRowSelected.firstName + " " + this.tableRowSelected.lastName) +
          " ) ";
      });
  ...
  isExpansionDetailRow = (index: number, row: CrmContactModel) =>
