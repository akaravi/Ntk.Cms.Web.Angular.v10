...
export class TicketingTaskWidgetComponent implements OnInit, OnDestroy {
  ...
  onActionStatist(): void {
    this.translate
      .get("MESSAGE.Get_unread_tickets_statistics")
    ...
    this.translate
      .get("MESSAGE.Get_read_tickets_statistics")
      .subscribe((str: string) => {
        this.publicHelper.processService.processStart(
          this.constructor.name + "Read",
          str,
          this.constructorInfoAreaId,
        );
      });
    this.translate
      .get("MESSAGE.Get_answered_tickets_statistics")
      .subscribe((str: string) => {
        this.publicHelper.processService.processStart(
          this.constructor.name + "Answered",
          str,
          this.constructorInfoAreaId,
        );
      });
    this.translate
      .get("MESSAGE.Get_statistics_on_all_tickets")
      .subscribe((str: string) => {
        this.publicHelper.processService.processStart(
    ...
    this.widgetInfoModel.setItem(
      new WidgetContentInfoModel("Unread", 0, 0, ""),
    );
    this.widgetInfoModel.setItem(new WidgetContentInfoModel("Read", 1, 0, ""));
    this.widgetInfoModel.setItem(
      new WidgetContentInfoModel("Answered", 2, 0, ""),
    );
        ...
        if (ret.isSuccess) {
          this.widgetInfoModel.setItem(
            new WidgetContentInfoModel(
              ...
              1,
              ret.totalRowCount,
              "/ticketing/task/listTicketStatus/" + TicketStatusEnum.Read,
            ),
          );
        }
        this.publicHelper.processService.processStop(
        ...
        if (ret.isSuccess) {
          this.widgetInfoModel.setItem(
            new WidgetContentInfoModel(
              ...
              2,
              ret.totalRowCount,
              "/ticketing/task/listTicketStatus/" + TicketStatusEnum.Answered,
            ),
          );
        }
        this.publicHelper.processService.processStop(
