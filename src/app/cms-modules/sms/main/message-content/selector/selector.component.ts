...
export class SmsMainMessageContentSelectorComponent implements OnInit {
  ...
  dataModelSelect: SmsMainMessageContentModel =
  ...
  @Input() set optionLinkCategryId(x: string) {
    if (x == this.privateLinkCategryId) return;
    if (this.privateLinkCategryId != "") this.onActionSelectClear();
  ...
  ): Promise<SmsMainMessageContentModel[]> {
  ...
  push(
    newvalue: SmsMainMessageContentModel,
  ): Observable<SmsMainMessageContentModel[]> {
