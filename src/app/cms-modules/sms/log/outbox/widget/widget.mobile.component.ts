...
@Component({
  selector: "app-sms-log-outbox-widget-mobile",
  templateUrl: "./widget.mobile.component.html",
  styleUrls: ["./widget.mobile.component.scss"],
  standalone: false,
})
export class SmsLogOutBoxWidgetMobileComponent implements OnInit, OnDestroy {
  @Input() cssClass = "";
  ...
  async onActionStatist() {
    this.translate.get("MESSAGE.outbox_list").subscribe((str: string) => {
    ...
    forkJoin([s0, s1, s2, s3]).subscribe((results) => {
      ...
      this.translate.get("TITLE.Sent_Successfully").subscribe((str: string) => {
        labels[3] = str;
      });
      if (ret.isSuccess) {
        this.widgetInfoModel.setItem(
