...
@Component({
  selector: "app-sms-action-send-message-api-mobile",
  templateUrl: "./send-message-api.mobile.component.html",
  styleUrls: ["./send-message-api.mobile.component.scss"],
  standalone: false,
})
export class SmsActionSendMessageApiMobileComponent implements OnInit {
  static nextId = 0;
  id = ++SmsActionSendMessageApiMobileComponent.nextId;
  ...
  dataPathModelResult: ErrorExceptionResult<SmsMainApiPathModel> =
    new ErrorExceptionResult<SmsMainApiPathModel>();
  dataNumberModelResult: ErrorExceptionResult<SmsMainApiNumberModel> =
  ...
  loadOptions(): void {
    this.filteredPathOptions = this.formPathControl.valueChanges.pipe(
      startWith(""),
      debounceTime(1500),
      distinctUntilChanged(),
      switchMap((val) => {
    ...
    this.filteredNumberOptions = this.formNumberControl.valueChanges.pipe(
      startWith(""),
      debounceTime(1500),
      distinctUntilChanged(),
      switchMap((val) => {
  ...
  async DataPathGetAll(
    text: string | number | any,
  ): Promise<SmsMainApiPathModel[]> {
    ...
    return await firstValueFrom(
      this.pathService.ServiceGetAll(filterModel),
    ).then((response) => {
      this.dataPathModelResult = response;
      /*select First Item */
      if (
        this.optionSelectFirstItem &&
        (!this.dataPathModelSelect ||
          !this.dataPathModelSelect.id ||
          this.dataPathModelSelect.id.length <= 0) &&
        this.dataPathModelResult.listItems.length > 0
      ) {
        this.optionSelectFirstItem = false;
        setTimeout(() => {
          this.formPathControl.setValue(this.dataPathModelResult.listItems[0]);
        }, 1000);
  ...
  async DataNumberGetAll(
    text: string | number | any,
  ): Promise<SmsMainApiNumberModel[]> {
    const filterModel = new FilterModel();
    filterModel.rowPerPage = 20;
    filterModel.accessLoad = true;
    let filter = new FilterDataModel();
    ...
    return await firstValueFrom(
      this.numberService.ServiceGetAll(filterModel),
    ).then((response) => {
      this.dataNumberModelResult = response;
      /*select First Item */
      if (
        this.optionSelectFirstItem &&
        (!this.dataNumberModelSelect ||
          !this.dataNumberModelSelect.id ||
          this.dataNumberModelSelect.id.length <= 0) &&
        this.dataNumberModelResult.listItems.length > 0
      ) {
        this.optionSelectFirstItem = false;
        setTimeout(() => {
          this.formNumberControl.setValue(
            this.dataNumberModelResult.listItems[0],
          );
        }, 1000);
  ...
  pushPath(newvalue: SmsMainApiPathModel): Observable<SmsMainApiPathModel[]> {
    return this.filteredPathOptions.pipe(
      map((items) => {
  ...
  pushNumber(
    newvalue: SmsMainApiNumberModel,
  ): Observable<SmsMainApiNumberModel[]> {
    return this.filteredNumberOptions.pipe(
      map((items) => {
        if (items.find((x) => x.id === newvalue.id)) {
          return items;
        }
  ...
  onActionSelectPathForce(id: string | SmsMainApiPathModel): void {
    if (typeof id === "string" && id.length > 0) {
      ...
      if (
        ...
        this.dataPathModelResult.listItems &&
        this.dataPathModelResult.listItems.find((x) => x.id === id)
      ) {
        const item = this.dataPathModelResult.listItems.find(
          (x) => x.id === id,
        );
  ...
  onActionSelectNumberForce(id: string | SmsMainApiNumberModel): void {
    if (typeof id === "string" && id.length > 0) {
      ...
      if (
        this.dataNumberModelResult &&
        this.dataNumberModelResult.listItems &&
        this.dataNumberModelResult.listItems.find((x) => x.id === id)
      ) {
        const item = this.dataNumberModelResult.listItems.find(
          (x) => x.id === id,
        );
