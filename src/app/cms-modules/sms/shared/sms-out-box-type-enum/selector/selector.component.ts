...
@Component({
  selector: "app-sms-out-box-type-enum-selector",
  templateUrl: "./selector.component.html",
  standalone: false,
})
export class SmsOutBoxTypeEnumSelectorComponent implements OnInit, OnDestroy {
  static nextId = 0;
  ...
  async DataGetAll(text: string | number | any): Promise<InfoEnumModel[]> {
    ...
    return await firstValueFrom(
      this.enumService.ServiceSmsMessageTypeEnum(),
    ).then((response) => {
      ...
      if (
        ...
        (!this.dataModelSelect ||
          !this.dataModelSelect.value ||
          this.dataModelSelect.value < 0) &&
  ...
  onActionSelectForce(id: string | InfoEnumModel): void {
    ...
    if (typeof id === "number" && id >= 0) {
      ...
      if (
        ...
        this.dataModelResult.listItems.find((x) => x.value === id)
      ) {
