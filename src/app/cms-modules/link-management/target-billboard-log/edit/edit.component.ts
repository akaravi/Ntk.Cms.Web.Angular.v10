...
import {CoreEnumService,
  ErrorExceptionResultBase,LinkManagementBillboardModel,
...
export class LinkManagementTargetBillboardLogEditComponent
  ...
  implements OnInit
{
  ...
  onActionSelectorLinkManagementBillboardId(
    model: LinkManagementBillboardModel | null,
  ): void {
    if (!model || !model.id || model.id <= 0) {
      this.translate
        .get("MESSAGE.Information_billboard_is_not_clear")
        .subscribe((str: string) => {
  ...
  onActionSelectorLinkManagementTargetId(
    model: LinkManagementTargetModel | null,
  ): void {
    if (!model || !model.id || model.id <= 0) {
      this.translate
        .get("MESSAGE.Information_target_is_not_clear")
        .subscribe((str: string) => {
          this.cmsToastrService.typeErrorSelected(str);
        });
      return;
