...
export class LinkManagementBillboardPatternSelectorComponent implements OnInit {
  ...
  dataModelManagementContentSettingTypeEnumResult: ErrorExceptionResult<InfoEnumModel> =
  ...
  displayFn(model?: LinkManagementBillboardPatternModel): string | undefined {
    if (
      this.dataModelManagementContentSettingTypeEnumResult?.listItems &&
      this.dataModelManagementContentSettingTypeEnumResult.listItems.length > 0
    ) {
      const find =
        this.dataModelManagementContentSettingTypeEnumResult.listItems.find(
          (x) =>
            x.key === model.settingType.toString() ||
            x.value === model.settingType,
        );
      if (find) return model ? model.title + "-" + find.description : undefined;
    }
    return model ? model.title : undefined;
  }
  displayOption(
    model?: LinkManagementBillboardPatternModel,
  ): string | undefined {
    if (
      this.dataModelManagementContentSettingTypeEnumResult?.listItems &&
      this.dataModelManagementContentSettingTypeEnumResult.listItems.length > 0
  ...
  ): Promise<LinkManagementBillboardPatternModel[]> {
  ...
  push(
    newvalue: LinkManagementBillboardPatternModel,
  ): Observable<LinkManagementBillboardPatternModel[]> {
  ...
  onActionSelectForce(id: number | LinkManagementBillboardPatternModel): void {
    ...
    if (typeof id === typeof LinkManagementBillboardPatternModel) {
      this.filteredOptions = this.push(
        id as LinkManagementBillboardPatternModel,
      );
