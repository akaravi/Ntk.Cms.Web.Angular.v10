...
export class LinkManagementBillboardPatternListComponent
  ...
  implements OnInit, OnDestroy
{
  ...
  onActionButtonRowTarget(
    ...
    if (event?.ctrlKey) {
      var link =
        "/#/linkmanagement/target/list/LinkBillboardPatternId/" + model.id;
      window.open(link, "_blank");
    } else {
      this.router.navigate([
        "/linkmanagement/target/list/LinkBillboardPatternId/",
        model.id,
      ]);
    }
  }
  onActionButtonRowBillboard(
    model: LinkManagementBillboardPatternModel = this.tableRowSelected,
    event?: MouseEvent,
  ): void {
    if (!model || !model.id || model.id === 0) {
    ...
    if (event?.ctrlKey) {
      var link =
        "/#/linkmanagement/billboard/list/LinkBillboardPatternId/" + model.id;
      window.open(link, "_blank");
    } else {
      this.router.navigate([
        "/linkmanagement/billboard/list/LinkBillboardPatternId/",
        model.id,
      ]);
    }
  }
