...
export class ApplicationLogNotificationActionSendComponent implements OnInit {
  ...
  dataModelResult: ErrorExceptionResult<ApplicationLogNotificationModel> =
    new ErrorExceptionResult<ApplicationLogNotificationModel>();
  dataModel: SendNotificationModel = new SendNotificationModel();
  dataModelEnumContentTypeResult: ErrorExceptionResult<InfoEnumModel> =
  ...
  DataAddContent(): void {
    ...
    this.applicationLogNotificationService
  ...
  onActionSelectApp(model: ApplicationAppModel | null): void {
    if (!model || model.id <= 0) {
      this.translate
        .get([
          "MESSAGE.Specify_the_application",
          "MESSAGE.Application_information_is_not_clear",
        ])
        .subscribe((str: any) => {
          this.cmsToastrService.typeErrorMessage(
            str["MESSAGE.Specify_the_application"],
            str["MESSAGE.Application_information_is_not_clear"],
          );
        });
      return;
    }
    ...
    if (!model || !model.id || model.id.length === 0) {
      this.translate
        .get([
          ...
          "MESSAGE.Information_application_member_is_not_clear",
        ])
        .subscribe((str: any) => {
          this.cmsToastrService.typeErrorMessage(
            str["MESSAGE.Specify_the_member_of_application"],
            str["MESSAGE.Information_application_member_is_not_clear"],
          );
        });
      return;
    }
  ...
  onFormSubmit(): void {
    ...
    if (
      ...
      this.translate
        .get([
          "MESSAGE.Specify_the_recipient",
          "MESSAGE.Application_or_user_to_receive_has_not_been_specified",
        ])
        .subscribe((str: any) => {
          this.cmsToastrService.typeErrorMessage(
            str["MESSAGE.Specify_the_recipient"],
            str[
              "MESSAGE.Application_or_user_to_receive_has_not_been_specified"
            ],
          );
