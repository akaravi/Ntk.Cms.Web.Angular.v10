...
export class ApplicationAppAddComponent
  extends AddBaseComponent<ApplicationAppService, ApplicationAppModel, number>
  implements OnInit
{
  ...
  dataModelEnumLangResult: ErrorExceptionResult<InfoEnumModel> =
  ...
  onFormSubmit(): void {
    ...
    if (this.dataModel.linkThemeConfigId <= 0) {
      this.translate
        .get("MESSAGE.Specify_the_application_format")
        .subscribe((str: string) => {
          this.cmsToastrService.typeErrorAdd(str);
        });
      return;
  ...
  DataAddContent(): void {
    ...
    this.applicationAppService.ServiceAdd(this.dataModel).subscribe({
      next: (ret) => {
  ...
  onActionSelectTheme(model: ApplicationThemeConfigModel | null): void {
    if (!model || model.id <= 0) {
      this.translate
        .get([
          "MESSAGE.Information_application_format_is_not_clear",
          "MESSAGE.Specify_the_template",
        ])
        .subscribe((str: any) => {
          this.cmsToastrService.typeErrorMessage(
            str["MESSAGE.Information_application_format_is_not_clear"],
