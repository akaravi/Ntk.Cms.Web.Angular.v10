...
export class ApplicationAppListComponent
  extends ListBaseComponent<ApplicationAppService, ApplicationAppModel, number>
  implements OnInit, OnDestroy
{
  ...
  ngOnInit(): void {
    ...
    this.requestLinkThemeConfigId = +Number(
      this.activatedRoute.snapshot.paramMap.get("LinkThemeConfigId"),
    );
  ...
  onActionButtonNewRow(): void {
    ...
    if (sourceId === 0) {
      this.translate
        .get("MESSAGE.Application_source_type_is_not_selected")
        .subscribe((str: string) => {
  ...
  onActionButtonEditRow(
    mode: ApplicationAppModel = this.tableRowSelected,
  ): void {
  ...
  onActionButtonMemberList(
    ...
    this.onActionTableRowSelect(mode);
    this.router.navigate([
      "/application/memberinfo/LinkApplicationId/",
      this.tableRowSelected.id,
    ]);
  }
  onActionButtonIntroList(
    mode: ApplicationAppModel = this.tableRowSelected,
  ): void {
    if (mode == null || !mode.id || mode.id === 0) {
      this.translate
    ...
    this.onActionTableRowSelect(mode);
    this.router.navigate([
      "/application/intro/LinkApplicationId/",
      this.tableRowSelected.id,
    ]);
  }
}
