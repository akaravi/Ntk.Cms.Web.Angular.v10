...
export class ArticleContentEditComponent
  extends EditBaseComponent<ArticleContentService, ArticleContentModel, number>
  implements OnInit
{
  requestId = 0;
  ...
  dataContentTagModelResult: ErrorExceptionResult<ArticleContentTagModel> =
    new ErrorExceptionResult<ArticleContentTagModel>();
  dataContentSimilarModelResult: ErrorExceptionResult<ArticleContentSimilarModel> =
    new ErrorExceptionResult<ArticleContentSimilarModel>();
  ...
  otherInfoTabledataSource =
    new MatTableDataSource<ArticleContentOtherInfoModel>();
  ...
  onActionContentSimilarRemoveFromLIst(
    model: ArticleContentModel | null,
  ): void {
    if (!model || model.id <= 0) {
      return;
    }
