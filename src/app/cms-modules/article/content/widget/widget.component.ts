...
export class ArticleContentWidgetComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.translate.get('TITLE.Registered_Atricle').subscribe((str: string) => { this.widgetInfoModel.title = str });
  ...
  onActionStatist(): void {
    this.translate.get('MESSAGE.Get_active_article_statistics').subscribe((str: string) => {
      this.publicHelper.processService.processStart(this.constructor.name + 'Active', str, this.constructorInfoAreaId);
    });
    this.translate.get('MESSAGE.Get_statistics_on_all_articles').subscribe((str: string) => {
