...
export class ArticleCommentListComponent
  extends ListBaseComponent<ArticleCommentService, ArticleCommentModel, number>
  implements OnInit, OnDestroy
{
  ...
  ngOnInit(): void {
    this.requestContentId = +Number(
      this.activatedRoute.snapshot.paramMap.get("ContentId"),
    );
  ...
  onActionButtonNewRow(): void {
    if (this.requestContentId == null || this.requestContentId === 0) {
      this.translate
  ...
  onActionButtonViewContent(model: ArticleCommentModel): void {
    ...
    this.translate
      .get("MESSAGE.get_article_information")
      .subscribe((str: string) => {
        this.publicHelper.processService.processStart(
          pName,
          str,
  ...
  onActionButtonEditContent(
    ...
    if (event?.ctrlKey) {
      this.link =
        "/#/article/content/edit/" + this.tableRowSelected.linkContentId;
      window.open(this.link, "_blank");
    } else {
      this.router.navigate([
        "/article/content/edit",
        this.tableRowSelected.linkContentId,
      ]);
    }
  }
