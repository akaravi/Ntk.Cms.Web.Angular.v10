...
import {File360TourDefaultModel,
...
export class Cms360TourListComponent implements OnInit, AfterViewInit {
  static nextId = 0;
  ...
  @Output() dataModelChange: EventEmitter<File360TourModel> =
    new EventEmitter<File360TourModel>();
  @Input() set dataModel(model: File360TourModel) {
    if (!model) {
      model = new File360TourModel();
    }
    if (!model.defaultValue) model.defaultValue = new File360TourDefaultModel();
    if (!model.scenes) model.scenes = new Map<string, File360TourScenesModel>();
    this.scenesList = [];
    ...
    this.scenesList.forEach((element) => {
      if (!element.hotSpots) element.hotSpots = [];
  ...
  uniqByReduce<T>(array: T[]) {
    ...
    array.forEach((item) => {
      if (
        ...
        result.filter(
          (x) => x["pitch"] == item["pitch"] && x["yaw"] == item["yaw"],
        ).length == 0
      ) {
        result.push(item);
      }
  ...
  actionPannellumImageLoad(
    ...
    if (this.viewer) this.onActionPannellumDestroy();
  ...
  onActionPannellumClickLastPoint(): void {
    if (
      this.postionView &&
      (this.postionView.clickGetYaw != 0 || this.postionView.clickGetPitch != 0)
    ) {
  ...
  onActionSubmitView360(): void {
    if (
      !this.dataDetailModel.linkFileId ||
      this.dataDetailModel.linkFileId <= 0
    ) {
  ...
  actionPrivateDataModelOptimaze() {
    ...
    if (!this.scenesList) this.scenesList = [];
    this.scenesList.forEach((element) => {
    ...
    if (
      !this.privateDataModel.defaultValue.firstScene ||
      this.privateDataModel.defaultValue.firstScene.length == 0
    ) {
      if (
        this.privateDataModel.scenes &&
        this.scenesList &&
        this.scenesList.length > 0
      )
        this.privateDataModel.defaultValue.firstScene =
          this.scenesList[0].linkFileId + "";
  ...
  onActionShowHotspotAdd(): void {
    if (!this.dataDetailModel)
      this.dataDetailModel = new File360TourScenesModel();
    if (!this.dataDetailModel.hotSpots) this.dataDetailModel.hotSpots = [];
    ...
    this.tableHotSpotdataSource.data = this.uniqByReduce(
  ...
  onActionOptionEditView360(index: number): void {
    ...
    this.actionPannellumImageLoad(
      this.dataDetailModel.panorama,
      this.dataDetailModel.hotSpots,
    );
  ...
  editHotspot: File360TourHotSpotModel;
  oldHotspot: File360TourHotSpotModel;
  ...
  removeROw(usr: File360TourHotSpotModel) {
    const indexId = this.dataDetailModel.hotSpots.findIndex(
      (x) => x.guid == usr.guid,
    );
    if (indexId >= 0) {
  ...
  updateEdit() {
    ...
    const indexId = this.dataDetailModel.hotSpots.findIndex(
      (x) => x.guid == this.oldHotspot.guid,
    );
    if (indexId >= 0) this.dataDetailModel.hotSpots[indexId] = this.editHotspot;
  ...
  cancelEdit() {
    ...
    if (this.inAddhotspotGuid?.length > 0) {
      this.dataDetailModel.hotSpots = this.dataDetailModel.hotSpots.filter(
        (x) => x.guid != this.inAddhotspotGuid,
      );
    }
    this.editHotspot = new File360TourHotSpotModel();
  ...
  onActionImportFile360View(): void {
    if (this.privateDataImageModel && this.privateDataImageModel.length > 0) {
      ...
      this.privateDataImageModel.forEach((element) => {
        if (
          !this.privateDataModel.scenes[element.linkFileId] &&
          element.linkFileId > 0
        ) {
