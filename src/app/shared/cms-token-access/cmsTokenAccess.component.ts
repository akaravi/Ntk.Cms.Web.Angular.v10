      ...
      next: (ret) => {
        ...
        if (ret) {
          this.translate
            .get([
              "TITLE.Information",
              "MESSAGE.Access_is_approved",
              "MESSAGE.New_access_not_approved",
            ])
            .subscribe((str: any) => {
        ...
        if (ret?.isSuccess) {
          this.translate
            ...
            .subscribe((str: any) => {
              const etitle = str["TITLE.Information"];
              if (
                ret.item.access.userAccessAdminAllowToProfessionalData ===
                NewToPerf
              ) {
          ...
          if (ret.item.access.userId === +this.inputUserId) {
            this.translate
              .get([
                ...
                "MESSAGE.Access_to_the_new_user_was_not_approved",
              ])
              .subscribe((str: any) => {
                if (this.cmsToastrService)
            ...
            this.translate
              .get("MESSAGE.Access_to_the_new_user_was_not_approved")
              .subscribe((str: string) => {
                if (this.cmsToastrService)
