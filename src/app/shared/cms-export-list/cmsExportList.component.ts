...
export class CmsExportListComponent implements OnInit {
  ...
  constructor(
    ...
    if (data) {
      if (data.service) this.requestService = data.service;
      else this.dialogRef.close({ dialogChangedDate: true });
  ...
  dataModelSubmitResult: ErrorExceptionResultExportFile =
    new ErrorExceptionResultExportFile();
  dataModelFileSelect: CoreModuleEntityReportFileModel =
  ...
  ngOnInit(): void {
    ...
    this.filterModel.exportFile.dataRowOrientation =
  ...
  DataGetAll(): void {
    ...
    this.requestService.ServiceReportFileGetAll().subscribe({
      next: (ret) => {
        ...
        if (ret.isSuccess) {
          this.dataModelReportFileResult.listItems =
  ...
  onOpenPage(): void {
    if (
      this.dataModelSubmitResult &&
      this.dataModelSubmitResult.isSuccess &&
      this.dataModelSubmitResult.linkFile.length > 0
    )
