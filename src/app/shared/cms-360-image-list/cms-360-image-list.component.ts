...
export class Cms360ImageListComponent implements OnInit, AfterViewInit {
  static nextId = 0;
  ...
  @Output() dataModelChange: EventEmitter<File360ViewModel[]> =
    new EventEmitter<File360ViewModel[]>();
  ...
  actionPrivateDataModelOptimaze() {
    ...
    if (!this.privateDataModel) this.privateDataModel = [];
    this.privateDataModel.forEach((element) => {
      if (!element.hotSpots) element.hotSpots = [];
  ...
  onActionShowHotspotAdd(): void {
    if (!this.dataDetailModel) this.dataDetailModel = new File360ViewModel();
