...
| H3  | **اسکرول بی‌نهایت**       | روی تگ **`<app-cms-html-list-mobile>`** باید بایندینگ **`(optionOnScrollNearBottom)="onActionLoadNextPage()"`** وجود داشته باشد. **استثنا:** فقط دو لیست **core-log/micro-service-ping** و **core-log/micro-service-status** که از ListBaseComponent ارث نمی‌برند این بایندینگ را **ندارند**.                                                                                                  | جستجو: `optionOnScrollNearBottom` |
...
- **تاریخ:** 2026-02-20
- **optionOnScrollNearBottom:** از 208 فایل list.mobile.component.html فقط **60 فایل** این بایندینگ را دارند. **148 فایل** فاقد آن هستند که از این میان **۲ مورد** (core-log/micro-service-ping و core-log/micro-service-status) استثنا هستند؛ یعنی **146 فایل** باید `(optionOnScrollNearBottom)="onActionLoadNextPage()"` دریافت کنند.
...
- **نتیجه build/lint:** (پس از اجرای build پر شود)
